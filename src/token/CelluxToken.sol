...
/**
 * @title CelluxToken
 * @notice CELL - Cellux platform governance and utility token
 * @dev Total supply: 10,000,000 CELL
 *      Distribution:
 ...
 *      - Treasury: 10% (1M)
 */
contract CelluxToken is ERC20, Ownable {
    uint256 public constant TOTAL_SUPPLY = 10_000_000 * 10 ** 18; // 10 million CELL
    ...
    constructor() ERC20("Cellux Token", "CELL") Ownable(msg.sender) {
    ...
    function initialize(address treasury, address team, address stakingVault, address liquidityMining)
        ...
        onlyOwner
    {
        require(!isInitialized, "CelluxToken: Already initialized");
        require(treasury != address(0), "CelluxToken: Invalid treasury");
        require(team != address(0), "CelluxToken: Invalid team");
        require(stakingVault != address(0), "CelluxToken: Invalid staking vault");
        require(liquidityMining != address(0), "CelluxToken: Invalid liquidity mining");
        ...
        // Verify total supply
        require(totalSupply() == TOTAL_SUPPLY, "CelluxToken: Distribution mismatch");
