...
/**
 * @title CelluxStaking
 * @notice Stake CELL tokens to earn USDC rewards from protocol fees
 * @dev 30% of protocol trading fees are distributed to CELL stakers
 */
contract CelluxStaking is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IERC20 public immutable celluxToken;
    IERC20 public immutable usdc;

    // Staking info
    struct StakeInfo {
        uint256 amount; // Amount of CELL staked
    ...
    constructor(address _celluxToken, address _usdc) Ownable(msg.sender) {
        require(_celluxToken != address(0), "CelluxStaking: Invalid CELL");
        require(_usdc != address(0), "CelluxStaking: Invalid USDC");

        celluxToken = IERC20(_celluxToken);
        usdc = IERC20(_usdc);
    }

    /**
     * @notice Stake CELL tokens
     * @param amount Amount of CELL to stake
     */
    function stake(uint256 amount) external nonReentrant {
        require(amount >= minStakeAmount, "CelluxStaking: Below minimum stake");
        ...
        // Transfer CELL from user
        celluxToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    /**
     * @notice Unstake CELL tokens
     * @param amount Amount of CELL to unstake
     */
    function unstake(uint256 amount) external nonReentrant {
        StakeInfo storage userStake = stakes[msg.sender];
        require(amount > 0, "CelluxStaking: Invalid amount");
        require(userStake.amount >= amount, "CelluxStaking: Insufficient stake");
        ...
        // Transfer CELL back to user (minus penalty if applicable)
        uint256 amountToReturn = amount - penalty;
        celluxToken.safeTransfer(msg.sender, amountToReturn);

        // Penalty goes to protocol treasury (burned or redistributed)
        if (penalty > 0) {
            celluxToken.safeTransfer(owner(), penalty);
        }
    ...
    function claimRewards() external nonReentrant {
        ...
        require(pending > 0, "CelluxStaking: No rewards to claim");
    ...
    function addRewards(uint256 amount) external nonReentrant {
        require(amount > 0, "CelluxStaking: Invalid amount");
        require(totalStaked > 0, "CelluxStaking: No stakers");
     ...
     * @return _totalStaked Total CELL staked
    ...
    function calculateAPR(uint256 rewardsPer7Days) external view returns (uint256 apr) {
        ...
        // Assume CELL = $1 for simplicity (can be made dynamic)
    ...
    function updateParameters(uint256 _minStakeAmount, uint256 _lockPeriod, uint256 _earlyUnstakePenaltyBps)
        ...
        onlyOwner
    {
        require(_minStakeAmount > 0, "CelluxStaking: Invalid min stake");
        require(_lockPeriod <= 90 days, "CelluxStaking: Lock too long");
        require(_earlyUnstakePenaltyBps <= 2500, "CelluxStaking: Penalty too high"); // Max 25%
    ...
    function emergencyWithdraw(address token, address to, uint256 amount) external onlyOwner {
        require(token != address(0), "CelluxStaking: Invalid token");
        require(to != address(0), "CelluxStaking: Invalid address");
        require(amount > 0, "CelluxStaking: Invalid amount");
